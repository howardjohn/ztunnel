...
async fn test_shutdown_lifecycle() {
	...
	let (app, _shutdown) =
		tokio::join!(time::timeout(Duration::from_secs(5), app.wait_termination()), shutdown.shutdown_now());
...
// Check that port conflicts on any address results in the app failing instead of silently failing
async fn test_bind_conflict_address<F: FnOnce(&mut ztunnel::config::Config) -> &mut config::Address>(f: F) {
	helpers::initialize_telemetry();
	let l = TcpListener::bind("127.0.0.1:0").await.unwrap();
	let mut cfg = test_config();
	let sa = f(&mut cfg);
...
async fn test_shutdown_drain() {
	...
	let mut stream = ta
		.socks5_connect(DestinationAddr::Ip(dst), TEST_WORKLOAD_SOURCE.parse().unwrap())
		.await;
...
async fn test_quit_lifecycle() {
	...
	let (app, _shutdown) =
		tokio::join!(time::timeout(Duration::from_secs(5), app.wait_termination()), admin_shutdown(addr));
	app.expect("app shuts down")
		.expect("app exits without error");
}
...
fn on_demand_dns_assertions(metrics: ParsedMetrics) {
	{
		...
		assert!(m.to_owned().unwrap().len() == 1, "expected metric {metric} to have len(1)");
		...
		assert!(value == expected, "expected metric {metric} to be 1, was {:?}", value);
	}
}
...
async fn test_on_demand_dns_request() {
	...
	run_requests_test(&format!("{TEST_VIP_DNS}:80"), "", 2, Some(on_demand_dns_assertions), true).await;
}
...
async fn test_stats_exist() {
	testapp::with_app(test_config(), |app| async move {
		...
		for metric in &[
			...
			assert!(metrics.query(metric, &Default::default()).is_some(), "expected metric {metric}");
		}
