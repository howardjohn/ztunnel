...
pub fn generate_test_certs_at(
	...
	let cert = cert.pem();
	let mut workload = WorkloadCertificate::new(key.as_bytes(), cert.as_bytes(), vec![TEST_ROOT]).unwrap();
...
fn test_ca() -> Certificate {
	...
	let ca_param = CertificateParams::from_ca_cert_pem(std::str::from_utf8(TEST_ROOT).unwrap()).unwrap();
...
impl ServerCertProvider for MockServerCertProvider {
	async fn fetch_cert(&mut self) -> Result<Arc<ServerConfig>, TlsError> {
		let mut sc = ServerConfig::builder_with_provider(crate::tls::lib::provider())
			...
			.with_no_client_auth()
			.with_single_cert(self.0.cert_and_intermediates(), self.0.private_key.clone_key())
			.unwrap();
