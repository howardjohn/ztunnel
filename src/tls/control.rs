...
async fn root_to_store(root_cert: &RootCert) -> Result<rustls::RootCertStore, Error> {
	...
	match root_cert {
		...
		RootCert::Default => {
			let certs = rustls_native_certs::load_native_certs().map_err(|e| Error::InvalidRootCert(e.to_string()))?;
			roots.add_parsable_certificates(certs);
		}
	};
	Ok(roots)
...
impl ControlPlaneClientCertProvider for ControlPlaneAuthentication {
	async fn fetch_cert(&self) -> Result<ClientConfig, Error> {
		match self {
			ControlPlaneAuthentication::RootCert(root_cert) => control_plane_client_config(root_cert).await,
