...
impl Certificate {
	...
	pub fn names(&self) -> Vec<String> {
		...
		self.parsed()
			...
			.flat_map(|dn| {
				dn.iter()
					.map(|x| reg.get(x.attr_type()).unwrap().sn().to_string() + "/" + x.as_str().unwrap())
			})
			.chain(
				self.parsed()
					.subject_alternative_name()
...
fn parse_cert(mut cert: Vec<u8>) -> Result<Certificate, Error> {
	...
	Ok(Certificate { der: der.clone(), expiry: expiration(cert) })
}
...
impl WorkloadCertificate {
	pub fn new(key: &[u8], cert: &[u8], chain: Vec<&[u8]>) -> Result<WorkloadCertificate, Error> {
		...
		Ok(WorkloadCertificate { cert, chain, private_key: key, roots: Arc::new(roots) })
	}
	...
	pub fn server_config(&self) -> Result<ServerConfig, Error> {
		...
		let raw_client_cert_verifier =
			WebPkiClientVerifier::builder_with_provider(self.roots.clone(), crate::tls::lib::provider()).build()?;

		let client_cert_verifier = crate::tls::workload::TrustDomainVerifier::new(raw_client_cert_verifier, td);
	...
	pub fn outbound_connector(&self, identity: Vec<Identity>) -> Result<OutboundConnector, Error> {
		...
		cc.enable_sni = false;
		Ok(OutboundConnector { client_config: Arc::new(cc) })
	}
