...
impl TrustDomainVerifier {
	...
	fn verify_trust_domain(&self, client_cert: &CertificateDer<'_>) -> Result<(), rustls::Error> {
		...
		trace!("verifying client identities {ids:?} against trust domain {:?}", want_trust_domain);
		ids.iter()
			.find(|id| match id {
...
impl OutboundConnector {
	pub async fn connect(self, stream: TcpStream) -> Result<client::TlsStream<TcpStream>, io::Error> {
...
impl IdentityVerifier {
	fn verify_full_san(&self, server_cert: &CertificateDer<'_>) -> Result<(), rustls::Error> {
		...
		trace!("verifying server identities {id:?} against {:?}", self.identity);
...
impl ServerCertVerifier for IdentityVerifier {
	...
	fn verify_server_cert(
		...
		rustls::client::verify_server_cert_signed_by_trust_anchor(&cert, &self.roots, intermediates, now, algs.all)?;
