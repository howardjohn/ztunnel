...
impl WorkloadProxyManagerState {
		...
		match msg {
			WorkloadMessage::AddWorkload(poddata) => {
				...
				let Some(wli) = poddata.workload_info else {
					return Err(Error::ProtocolError("workload_info is required but not present".into()));
				};
				...
				let info =
					WorkloadInfo { name: wli.name, namespace: wli.namespace, service_account: wli.service_account };
			...
			WorkloadMessage::KeepWorkload(workload_uid) => {
				info!(uid = workload_uid.0, "pod keep received. will not delete it when snapshot is sent");
				if self.snapshot_received {
			...
			WorkloadMessage::DelWorkload(workload_uid) => {
				info!(uid = workload_uid.0, "pod delete request, shutting down proxy");
				if !self.snapshot_received {
	...
	pub async fn drain(self) {
		let drain_futures = self
			.workload_states
			.into_iter()
			.map(|(_, v)| v.drain.start_drain_and_wait(drain::DrainMode::Graceful) /* do not .await here!!! */);
	...
	async fn add_workload_inner(
		...
		if let Some(proxy) = proxies.proxy {
			tokio::spawn(
				...
				.instrument(
					tracing::info_span!("proxy", wl=%format!("{}/{}", workload_info.namespace, workload_info.name)),
				),
			);
		}
		if let Some(proxy) = proxies.dns_proxy {
			tokio::spawn(proxy.run().instrument(
				tracing::info_span!("dns_proxy", wl=%format!("{}/{}", workload_info.namespace, workload_info.name)),
			));
		}

		self.workload_states
			.insert(workload_uid.clone(), WorkloadState { drain: drain_tx, netns_id: workload_netns_id });
...
mod tests {
	...
	macro_rules! fixture {
		() => {{
			...
			let state =
				WorkloadProxyManagerState::new(f.proxy_factory, f.ipc, f.inpod_metrics.clone(), Default::default());
			Fixture { state, metrics: f.inpod_metrics }
		}};
	}
	...
	async fn add_workload_starts_a_proxy() {
		...
		let data = WorkloadData { netns: new_netns(), workload_uid: uid(0), workload_info: workload_info() };
		state
	...
	async fn idemepotency_add_workload_starts_only_one_proxy() {
		...
		let ns = new_netns();
		let data =
			WorkloadData { netns: ns.try_clone().unwrap(), workload_uid: uid(0), workload_info: workload_info() };
		state
			.process_msg(WorkloadMessage::AddWorkload(data))
			.await
			.unwrap();
		let data = WorkloadData { netns: ns, workload_uid: uid(0), workload_info: workload_info() };
		state
			.process_msg(WorkloadMessage::AddWorkload(data))
			.await
			.unwrap();
	...
	async fn add_delete_add_workload_starts_only_one_proxy() {
		...
		let msg3 =
			WorkloadMessage::AddWorkload(WorkloadData { netns: ns, workload_uid, workload_info: workload_info() });
	...
	async fn no_workload_info_rejected() {
		...
		let data = WorkloadData { netns: new_netns(), workload_uid: uid(0), workload_info: None };
