...
impl Default for Fixture {
	fn default() -> Fixture {
		...
		let proxy_gen =
			ProxyFactory::new(Arc::new(cfg), dstate, cert_manager, metrics, dns_metrics, drain_rx.clone()).unwrap();
		Fixture {
...
pub fn new_netns() -> OwnedFd {
	...
	std::thread::scope(|s| {
		s.spawn(|| {
			...
			if let Ok(newns) = std::fs::File::open(format!("/proc/self/task/{}/ns/net", nix::unistd::gettid())) {
...
pub async fn send_snap_sent(s: &mut UnixStream) {
	let r = WorkloadRequest {
		payload: Some(crate::inpod::istio::zds::workload_request::Payload::SnapshotSent(Default::default())),
	};
...
pub async fn send_workload_added(
	...
	let r = WorkloadRequest {
		payload: Some(crate::inpod::istio::zds::workload_request::Payload::Add(
			crate::inpod::istio::zds::AddWorkload { uid: uid.into_string(), workload_info: info },
		)),
	};
	...
	s.async_io(tokio::io::Interest::WRITABLE, || {
		nix::sys::socket::sendmsg::<()>(s.as_raw_fd(), &iov, &cmsgs[..], nix::sys::socket::MsgFlags::empty(), None)
...
pub async fn send_workload_del(s: &mut UnixStream, uid: super::WorkloadUid) {
	let r = WorkloadRequest {
		payload: Some(crate::inpod::istio::zds::workload_request::Payload::Del(
			crate::inpod::istio::zds::DelWorkload { uid: uid.into_string() },
		)),
	};
	let data: Vec<u8> = r.encode_to_vec();
	...
	s.async_io(tokio::io::Interest::WRITABLE, || {
		nix::sys::socket::sendmsg::<()>(s.as_raw_fd(), &iov, &[], nix::sys::socket::MsgFlags::empty(), None)
			.map_err(|e| std::io::Error::from_raw_os_error(e as i32))
	})
	.await
	.expect("failed to sendmsg");
...
pub fn create_proxy_confilct(ns: &std::os::fd::OwnedFd) -> std::os::fd::OwnedFd {
	let inpodns =
		InpodNetns::new(Arc::new(crate::inpod::netns::InpodNetns::current().unwrap()), ns.try_clone().unwrap())
			.unwrap();
	let tl = inpodns
