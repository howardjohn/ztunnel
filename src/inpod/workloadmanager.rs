...
impl WorkloadProxyReadinessHandler {
	fn new(ready: readiness::Ready, reconnect_backoff: Option<ExponentialBackoff>) -> Self {
		...
		let mut r = Self { ready, block_ready: None, backoff };
		r.not_ready();
		r
	}
	...
	fn not_ready(&mut self) {
		...
		if self.block_ready.is_none() {
			self.block_ready = Some(std::mem::ManuallyDrop::new(self.ready.register_task("workload proxy manager")));
		}
	}
}
		...
		loop {
			match super::packet::connect(&self.uds).await {
				Err(e) => {
					...
					warn!("failed to connect to the Istio CNI node agent over {:?}, is the node agent healthy? details: {:?}. retrying in {:?}", &self.uds, e, backoff);
...
impl WorkloadProxyManager {
	pub fn verify_syscalls() -> anyhow::Result<()> {
		...
		super::netns::InpodNetns::capable().map_err(|e| anyhow::anyhow!("failed to set netns: {:?}", e))?;
	...
	fn verify_set_mark() -> anyhow::Result<()> {
		...
		crate::socket::set_mark(&socket, 1337).map_err(|e| {
			anyhow::anyhow!(
				"failed to set mark on socket. make sure ztunnel has CAP_NET_RAW, CAP_NET_ADMIN. error: {:?}",
				e
			)
		})
	}
	...
	pub fn new(
		...
		let mgr =
			WorkloadProxyManager { state, networking, readiness: WorkloadProxyReadinessHandler::new(ready, None) };
		Ok(mgr)
	}
	...
	async fn run_internal(&mut self, drain: DrainWatcher) -> Result<(), anyhow::Error> {
		...
		let _rs = loop {
			...
			let mut processor_helper = WorkloadProxyManagerProcessor::new(&mut self.state, &mut self.readiness);
...
impl<'a> WorkloadProxyManagerProcessor<'a> {
	fn new(
		...
		Self { state, readiness, next_pending_retry: None }
	}
...
pub(crate) mod tests {
	...
	use crate::inpod::test_helpers::{
		self, create_proxy_confilct, new_netns, read_hello, read_msg, send_snap_sent, send_workload_added,
		send_workload_del, uid,
	};
	...
	macro_rules! fixture {
		() => {{
			...
			Fixture { state, inpod_metrics: f.inpod_metrics, drain_rx: f.drain_rx, _drain_tx: f.drain_tx }
		}};
	}
	...
	async fn test_process_failed() {
		let f = fixture!();
		let (s1, mut s2) = UnixStream::pair().unwrap();
		let processor: WorkloadStreamProcessor = WorkloadStreamProcessor::new(s1, f.drain_rx.clone());
		let mut state = f.state;
	...
	async fn test_process_snapshot_with_missing_workload() {
		...
		let key_set: HashSet<crate::inpod::WorkloadUid> = state.workload_states().keys().cloned().collect();
