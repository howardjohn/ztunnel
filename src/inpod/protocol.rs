...
impl WorkloadStreamProcessor {
	...
	pub async fn send_hello(&mut self) -> std::io::Result<()> {
		let r = ZdsHello { version: Version::V1 as i32 };
		self.send_msg(r).await
	}

	pub async fn send_ack(&mut self) -> std::io::Result<()> {
		let r = WorkloadResponse { payload: Some(zds::workload_response::Payload::Ack(Ack { error: String::new() })) };
		self.send_msg(r).await
	}
	pub async fn send_nack(&mut self, e: anyhow::Error) -> std::io::Result<()> {
		let r = WorkloadResponse { payload: Some(zds::workload_response::Payload::Ack(Ack { error: e.to_string() })) };
		self.send_msg(r).await
	}
		...
		let (flags, maybe_our_fd, len) = {
			...
			let res = loop {
				...
				let res = self.stream.try_io(tokio::io::Interest::READABLE, || {
					recvmsg::<()>(raw_fd, &mut iov, Some(msgspace_ref), MsgFlags::MSG_CMSG_CLOEXEC)
						.map_err(|e| std::io::Error::from_raw_os_error(e as i32))
				});
...
fn get_workload_data(
	...
	match (payload, maybe_our_fd) {
		...
		(Payload::Keep(k), None) => Ok(WorkloadMessage::KeepWorkload(super::WorkloadUid::new(k.uid))),
...
fn maybe_get_fd(cmsgs: impl Iterator<Item = ControlMessageOwned>) -> anyhow::Result<Option<std::os::fd::OwnedFd>> {
...
mod tests {
	...
	fn test_parse_add_workload() {
		...
		let data = prep_request(zds::workload_request::Payload::Add(istio::zds::AddWorkload {
	...
	fn test_parse_del_workload_with_fds_fails() {
		...
		let flags = MsgFlags::empty();
		let data =
			prep_request(zds::workload_request::Payload::Del(istio::zds::DelWorkload { uid: uid(0).into_string() }));
