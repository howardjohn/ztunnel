...
impl InpodNetns {
	pub fn current() -> std::io::Result<OwnedFd> {
		let curns = std::fs::File::open(format!("/proc/self/task/{}/ns/net", nix::unistd::gettid()));
		curns.map(|f| f.into())
	}
	...
	pub fn capable() -> std::io::Result<()> {
		...
		setns(curns, CloneFlags::CLONE_NEWNET).map_err(|e| std::io::Error::from_raw_os_error(e as i32))
	}
	...
	pub fn new(cur_netns: Arc<OwnedFd>, workload_netns: OwnedFd) -> std::io::Result<Self> {
		...
		let dev = res.st_dev;
		Ok(InpodNetns {
			inner: Arc::new(NetnsInner { cur_netns, netns: workload_netns, netns_id: NetnsID { inode, dev } }),
		})
	}
	...
	where
		F: FnOnce() -> T,
	{
		setns(&self.inner.netns, CloneFlags::CLONE_NEWNET).map_err(|e| std::io::Error::from_raw_os_error(e as i32))?;
		let ret = f();
...
mod tests {
	...
	fn new_netns() -> OwnedFd {
		...
		std::thread::scope(|s| {
			s.spawn(|| {
				...
				if let Ok(newns) = std::fs::File::open(format!("/proc/self/task/{}/ns/net", gettid())) {
	...
	fn test_run_works() {
		...
		let sync_netns = netns_rs::get_from_path(format!("/proc/self/fd/{}", other_netns.as_raw_fd())).unwrap();
		sync_netns
