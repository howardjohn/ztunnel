...
use nix::sys::socket::{
	bind as nixbind, connect as nixconnect, listen, socket, AddressFamily, SockFlag, SockType, UnixAddr,
};
use std::cmp;
...
pub fn bind(path: &Path) -> std::io::Result<UnixListener> {
	let socket =
		socket(AddressFamily::Unix, SockType::SeqPacket, SockFlag::SOCK_NONBLOCK | SockFlag::SOCK_CLOEXEC, None)?;
