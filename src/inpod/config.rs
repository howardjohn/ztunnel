...
impl InPodConfig {
	...
	pub fn socket_factory(&self, netns: InpodNetns) -> Box<dyn crate::proxy::SocketFactory + Send + Sync> {
...
impl InPodSocketFactory {
	...
	fn configure<S: std::os::unix::io::AsFd, F: FnOnce() -> std::io::Result<S>>(&self, f: F) -> std::io::Result<S> {
...
mod test {
	...
	macro_rules! fixture {
		() => {{
			...
			crate::config::Config { packet_mark: Some(123), ..crate::config::parse_config().unwrap() }
		}};
	}
	...
	async fn test_inpod_config_no_port_reuse() {
		...
		assert_eq!(inpod_cfg.mark(), Some(std::num::NonZeroU32::new(123).unwrap()));
		assert!(!inpod_cfg.reuse_port);

		let sf = inpod_cfg.socket_factory(
			InpodNetns::new(Arc::new(crate::inpod::netns::InpodNetns::current().unwrap()), new_netns()).unwrap(),
		);
		...
		let sock_addr: std::net::SocketAddr = "127.0.0.1:8080".parse().unwrap();
		{
			...
			assert_eq!(sock_ref.local_addr().unwrap(), socket2::SockAddr::from(sock_addr));
