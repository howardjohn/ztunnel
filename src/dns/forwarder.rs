...
impl Forwarder {
	...
	pub fn new(
		...
		let provider = GenericConnector::new(RuntimeProviderAdaptor { socket_factory, handle: Default::default() });
...
impl RuntimeProvider for RuntimeProviderAdaptor {
	...
	fn connect_tcp(&self, server_addr: SocketAddr) -> Pin<Box<dyn Send + Future<Output = std::io::Result<Self::Tcp>>>> {
		...
		Box::pin(async move {
			let socket = if server_addr.is_ipv4() { sf.new_tcp_v4() } else { sf.new_tcp_v6() }?;
...
mod tests {
	...
	async fn found() {
		...
		let f = run_dns(HashMap::from([(n("test.example.com."), vec![ip("1.1.1.1")])]))
			.await
			.unwrap();

		// Lookup a host.
		let req = a_request(n("test.example.com"), socket_addr("1.1.1.1:80"), Protocol::Udp);
	...
	async fn not_found() {
		...
		// Lookup a host.
		let req = a_request(n("fake-blahblahblah.com"), socket_addr("1.1.1.1:80"), Protocol::Udp);
