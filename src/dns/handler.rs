...
impl Handler {
	...
	async fn lookup<R: ResponseHandler>(&self, request: &Request, response_handle: R) -> ResponseInfo {
...
async fn send_lookup<R: ResponseHandler>(request: &Request, response_handle: R, answer: Answer) -> ResponseInfo {
	...
	// Build the response.
	let response = builder.build(response_header, answer.record_iter(), None.iter(), None.iter(), None.iter());

	// Send the response.
	send_response(response, response_handle).await
}

async fn send_lookup_error<R: ResponseHandler>(request: &Request, response_handle: R, e: LookupError) -> ResponseInfo {
	match e {
...
async fn send_error<R: ResponseHandler>(request: &Request, response_handle: R, code: ResponseCode) -> ResponseInfo {
	let response = MessageResponseBuilder::from_message_request(request).error_msg(request.header(), code);

	send_response(response, response_handle).await
}

/// Sends an empty response to the [ResponseHandler].
async fn send_empty_response<R: ResponseHandler>(request: &Request, response_handle: R) -> ResponseInfo {
	let empty = MessageResponseBuilder::from_message_request(request).build_no_records(*request.header());
