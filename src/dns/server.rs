...
use crate::dns::metrics::{DnsRequest, ForwardedDuration, ForwardedFailure, ForwardedRequest, Metrics};
...
impl Server {
	...
	pub async fn new(
		...
		let store = Store::new(domain, state, forwarder, metrics, local_workload_information);
		...
		for addr in local_address.into_iter() {
			...
			server.register_listener(tcp_listener.inner(), Duration::from_secs(DEFAULT_TCP_REQUEST_TIMEOUT));
...
impl Store {
	fn new(
		...
		Self { state, forwarder, domain, svc_domain, metrics, local_workload: local_workload_information }
	}
	...
	fn get_aliases(&self, client: &Workload, name: &Name) -> Vec<Alias> {
		...
		add_alias(Alias { name: name.clone(), stripped: None });
		...
		for kube_fqdn in self.to_kube_fqdns(name, &namespaced_domain) {
			add_alias(Alias { name: kube_fqdn, stripped: None });
		}
		...
		for search_domain in self.forwarder.search_domains(client) {
			if let Some(stripped_name) = trim_domain(name, &search_domain) {
				...
				add_alias(Alias {
					...
					stripped: Some(Stripped { name: stripped_name.clone(), search_domain: search_domain.clone() }),
				});
	...
	fn find_server(&self, client: &Workload, requested_name: &Name) -> Option<ServerMatch> {
		...
		trace!("checking aliases {:#?}", self.get_aliases(client, requested_name));
		for alias in self.get_aliases(client, requested_name) {
			...
			//   '*.example.com'
			//   '*.com'
			trace!("checking against wildcards {:#?}", get_wildcards(&alias.name));
			for mut search_name in get_wildcards(&alias.name) {
				...
				if let Some(service) = service {
					return Some(ServerMatch { server: Address::Service(Arc::new(service)), name: search_name, alias });
				}
	...
	fn get_addresses(&self, client: &Workload, server: &Address, record_type: RecordType) -> Vec<IpAddr> {
		let mut addrs: Vec<IpAddr> = match server {
			Address::Workload(wl) => wl
				.workload_ips
				.iter()
				.filter_map(|addr| if is_record_type(addr, record_type) { Some(*addr) } else { None })
				.collect(),
			Address::Service(service) => {
				if service.vips.is_empty() {
					...
					service
						...
						.iter()
						.filter_map(|vip| {
							if is_record_type(&vip.address, record_type) && client.network == vip.network {
								Some(vip.address)
							} else {
								None
							}
	...
	async fn forward(&self, client: Option<&Workload>, request: &Request) -> Result<Answer, LookupError> {
		// Increment counter for all requests.
		self.metrics
			.increment(&DnsRequest { request, source: client });

		// Increment counter for forwarded requests.
		self.metrics
			.increment(&ForwardedRequest { request, source: client });
		...
		let _forwarded_duration = self.metrics.defer_record(|metrics| {
			metrics.record(&ForwardedDuration { request, source: client }, start.elapsed());
		});
		...
		match self.forwarder.forward(client, request).await {
			...
			Err(e) => {
				...
				self.metrics
					.increment(&ForwardedFailure { request, source: client });
...
impl Resolver for Store {
	...
	async fn lookup(&self, request: &Request) -> Result<Answer, LookupError> {
		let client = self.local_workload.get_workload().await.map_err(|_| {
			...
			self.metrics
				.increment(&DnsRequest { request, source: None });
		...
		// Increment counter for all requests.
		self.metrics
			.increment(&DnsRequest { request, source: Some(&client) });
...
pub fn forwarder_for_mode(
	...
	Ok(match proxy_mode {
		...
		ProxyMode::Dedicated => Arc::new(SystemForwarder::new(false, cluster_domain, socket_factory)?),
	})
}
...
impl SystemForwarder {
	fn new(
		...
		Self::from_parts(per_pod, cluster_domain, socket_factory, opts, domain, search_domains, name_servers)
	}
	...
	fn from_parts(
		...
		let resolver = Arc::new(
			dns::forwarder::Forwarder::new(cfg, socket_factory, opts).map_err(|e| Error::Generic(Box::new(e)))?,
		);
		...
		Ok(Self { search_domains, resolver })
	}
}
	...
	fn search_domains(&self, wl: &Workload) -> Vec<Name> {
		...
		match &self.search_domains {
			...
			SearchDomains::Dynamic(cluster_domain) => vec![
				...
				Name::from_utf8(format!("svc.{cluster_domain}")).expect("inputs must be valid DNS labels"),
...
mod tests {
	...
	use crate::test_helpers::dns::{
		a, aaaa, cname, ip, ipv4, ipv6, n, new_message, new_tcp_client, new_udp_client, run_dns, send_request,
		server_request,
	};
	...
	fn test_to_kube_fqdns() {
		...
		let cases: &[Case] = &[
			...
			Case { host: "pod.sub-domain.ns.not-svc", client_namespace: "ns1", expected: vec![] },
			Case {
	...
	async fn lookup() {
		...
		let cases = [
			...
			Case {
				...
				host: "dual.localhost.",
				query_type: RecordType::AAAA,
				expect_records: vec![aaaa(n("dual.localhost."), ipv6("2001:db8:0:0:0:ff00:42:8329"))],
				..Default::default()
			},
			Case {
			...
			Case {
				...
				expect_records: vec![a(n("both-networks.ns1.svc.cluster.local."), ipv4("21.21.21.21"))],
				..Default::default()
			},
			Case {
			...
			Case {
				...
				query_type: RecordType::AAAA,
				expect_records: vec![aaaa(n("headless-ipv6.ns1.svc.cluster.local."), ipv6("2001:db8::33"))],
				..Default::default()
			},
			Case {
	...
	async fn forward_to_server() {
		...
		let cases = [
			Case { name: "success: www.google.com", host: "test.example.com.", expect_code: ResponseCode::NoError },
			Case {
		...
		let forwarder = Arc::new(
			run_dns(HashMap::from([(n("test.example.com."), vec![ip("1.1.1.1")])]))
				.await
				.unwrap(),
		);
		let (_signal, drain) = drain::new();
	...
	fn search_domains() {
		...
		let search =
			vec![as_name("istio-system.svc.cluster.local"), as_name("svc.cluster.local"), as_name("cluster.local")];
		...
		assert_eq!(
			...
			vec![as_name("default-ns.svc.cluster.local"), as_name("svc.cluster.local"), as_name("cluster.local"),]
		);
	}
		...
		let services = vec![
			...
			with_fqdn(
				...
				xds_service(
					"details",
					NS2,
					&[na(NW1, "11.11.11.11"), na(NW1, "12.12.12.12"), na(NW1, "13.13.13.13"), na(NW1, "14.14.14.14")],
				),
			),
			...
			xds_external_service("dual.localhost", &[na(NW1, "2.2.2.2"), na(NW1, "2001:db8:0:0:0:ff00:42:8329")]),
			// Wildcards.
			...
			xds_service("both-networks", NS1, &[na(NW1, "21.21.21.21"), na(NW2, "22.22.22.22")]),
			// Headless services.
	...
	fn na<S1: AsRef<str>, S2: AsRef<str>>(network: S1, addr: S2) -> NetworkAddress {
		NetworkAddress { network: strng::new(network.as_ref()), address: ip(addr) }
	}
	...
	fn xds_service<S1: AsRef<str>, S2: AsRef<str>>(name: S1, ns: S2, vips: &[NetworkAddress]) -> XdsService {
		XdsService {
			...
			addresses: vips
				.iter()
				.map(|vip| XdsNetworkAddress { network: vip.network.to_string(), address: addr_bytes(vip.address) })
				.collect(),
			ports: vec![XdsPort { service_port: 80, target_port: 80 }],
			..Default::default()
		}
	}

	fn xds_external_service<S: AsRef<str>>(hostname: S, addrs: &[NetworkAddress]) -> XdsService {
		with_fqdn(hostname.as_ref(), xds_service(hostname.as_ref(), NS1, addrs))
	}

	fn xds_workload(name: &str, ns: &str, host: &str, nw: &str, services: &[&str], ips: &[IpAddr]) -> XdsWorkload {
		XdsWorkload {
			addresses: ips
				.iter()
			...
			services: {
				...
				for service in services {
					out.insert(
						service.to_string(),
						XdsPortList { ports: vec![XdsPort { service_port: 80, target_port: 80 }] },
					);
				}
				out
			},
