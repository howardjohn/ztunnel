...
use crate::xds::istio::ca::istio_certificate_service_server::{IstioCertificateService, IstioCertificateServiceServer};
use crate::{
	tls,
...
impl CaServer {
	pub async fn spawn() -> (watch::Sender<Result<IstioCertificateResponse, tonic::Status>>, CaClient) {
		...
		let certs =
			tls::mock::generate_test_certs(&server_addr.ip().into(), Duration::from_secs(0), Duration::from_secs(100));
		...
		tokio::spawn(async move {
			while let Some(socket) = tls_stream.next().await {
				...
				if let Err(err) = crate::hyper_util::http2_server()
					.serve_connection(TokioIo::new(socket), hyper_tower::TowerToHyperService::new(srv))
					.await
				{
		...
		let client = CaClient::new(
			...
			AuthSource::Token(PathBuf::from(r"src/test_helpers/fake-jwt"), "Kubernetes".to_string()),
			true,
			60 * 60 * 24,
		)
		.await
