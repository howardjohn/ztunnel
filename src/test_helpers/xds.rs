...
impl AdsServer {
	pub async fn spawn(
		...
		let mut cfg =
			test_config_with_port_xds_addr_and_root_cert(80, Some(listener_addr_string), Some(root_cert), None);
		...
		let tls_client_fetcher = Box::new(tls::ControlPlaneAuthentication::RootCert(cfg.xds_root_cert.clone()));
...
impl AggregatedDiscoveryService for AdsServer {
	...
	async fn delta_aggregated_resources(
		...
		let conn = AdsConnection { rx: req_rx, tx: resp_tx };
