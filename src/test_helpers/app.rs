	...
	Fut: Future<Output = FO>,
{
	...
	info!("running with config: {}", serde_yaml::to_string(&cfg).unwrap());
...
impl TestApp {
	...
	pub async fn readiness_request(&self) -> anyhow::Result<()> {
		let req = Request::builder()
			.method(Method::GET)
			.uri(format!("http://localhost:{}/healthz/ready", self.readiness_address.port()))
			.body(Empty::<Bytes>::new())
			.unwrap();
		let client = hyper_util::pooling_client();
		let resp = client
		...
		match resp.status() {
			...
			other => Err(anyhow::anyhow!("non-200 status code from readiness request: received {}", other)),
		}
	}
	...
	pub async fn socks5_connect(&self, addr: DestinationAddr, source: IpAddr) -> TcpStream {
		...
		let socks_addr = with_ip(self.proxy_addresses.socks5.unwrap(), IpAddr::V4(Ipv4Addr::new(127, 0, 0, 1)));
	...
	pub async fn dns_request(&self, hostname: &str, udp: bool, ipv6: bool) -> hickory_proto::xfer::DnsResponse {
		let addr = if udp { self.udp_dns_proxy_address.unwrap() } else { self.tcp_dns_proxy_address.unwrap() };
		dns_request(addr, hostname, udp, ipv6).await
	}
}

pub async fn dns_request(addr: SocketAddr, hostname: &str, udp: bool, ipv6: bool) -> hickory_proto::xfer::DnsResponse {
	...
	let mut client = if udp { new_udp_client(addr).await } else { new_tcp_client(addr).await };

	let query_type = if ipv6 { RecordType::AAAA } else { RecordType::A };
...
pub async fn socks5_connect(mut stream: TcpStream, addr: DestinationAddr) -> anyhow::Result<TcpStream> {
	stream
		.write_all(&[
			0x05u8, // socks5
			0x1u8,  // 1 auth method
...
impl ParsedMetrics {
	pub fn query(&self, metric: &str, labels: &HashMap<String, String>) -> Option<Vec<&prometheus_parse::Sample>> {
		if !self
			.scrape
			.docs
