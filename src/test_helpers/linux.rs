...
impl WorkloadManager {
	...
	pub async fn deploy_dedicated_ztunnel(
		...
		let proxy_mode = if ztunnel_server.is_some() { ProxyMode::Shared } else { ProxyMode::Dedicated };
		...
		let cfg = config::Config {
			...
			require_original_source: if proxy_mode == ProxyMode::Dedicated { Some(false) } else { Some(true) },
		...
		tx_cfg.wait().await?;
		let zt_info = LocalZtunnel { fd_sender: ztunnel_server, config_sender: tx_cfg, namespace: ns };
...
impl<'a> TestWorkloadBuilder<'a> {
	...
	pub fn identity(mut self, identity: identity::Identity) -> Self {
		match identity {
			identity::Identity::Spiffe { trust_domain, namespace, service_account } => {
	...
	pub async fn register(mut self) -> anyhow::Result<Namespace> {
		...
		self.w.workload.uid =
			format!("cluster1//v1/Pod/{}/{}", self.w.workload.namespace, self.w.workload.name,).into();
		...
		if self.captured {
			...
			if self.manager.mode == Shared {
				...
				let msg = inpod::StartZtunnelMessage { uid: uid.to_string(), workload_info: Some(wli), fd };
				zt_info
					.fd_sender
					.as_mut()
					.unwrap()
