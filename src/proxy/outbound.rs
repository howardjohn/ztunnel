...
impl Outbound {
	...
	pub(super) async fn run(self) {
		...
		run_with_drain("outbound".to_string(), self.drain, pi.cfg.self_termination_deadline, accept).await
	}
}
...
impl OutboundConnection {
	async fn proxy(&mut self, source_stream: TcpStream) {
		let source_addr = socket::to_canonical(source_stream.peer_addr().expect("must receive peer addr"));
		let dst_addr = socket::orig_dst_addr_or_default(&source_stream);
		self.proxy_to(source_stream, source_addr, dst_addr).await;
	}

	pub async fn proxy_to(&mut self, source_stream: TcpStream, source_addr: SocketAddr, dest_addr: SocketAddr) {
		let start = Instant::now();
		...
		let _conn_guard = self
			.pi
			.connection_manager
			.track_outbound(source_addr, dest_addr, req.actual_destination);
	...
	async fn send_hbone_request(&mut self, remote_addr: SocketAddr, req: &Request) -> Result<H2Stream, Error> {
	...
	async fn proxy_to_tcp(
		...
		copy::copy_bidirectional(copy::TcpStreamSplitter(stream), copy::TcpStreamSplitter(outbound), connection_stats)
			.await
	}
	...
	async fn build_request(
		...
		let svc_addressed = if let Some(Address::Service(target_service)) = state
			.fetch_address(&NetworkAddress { network: self.pi.cfg.network.clone(), address: target.ip() })
			.await
		{
		...
		let Some(us) = state
			.fetch_upstream(source_workload.network.clone(), &source_workload, target, ServiceResolutionMode::Standard)
			.await?
		else {
			if svc_addressed {
		...
		let from_waypoint =
			proxy::check_from_waypoint(state, &us.workload, Some(&source_workload.identity()), &downstream).await;
...
fn baggage(r: &Request, cluster: String) -> String {
	format!(
		"k8s.cluster.name={cluster},k8s.namespace.name={namespace},k8s.{workload_type}.name={workload_name},service.name={name},service.version={version}",
...
mod tests {
	...
	async fn run_build_request(from: &str, to: &str, xds: XdsAddressType, expect: Option<ExpectedRequest<'_>>) {
	...
	async fn run_build_request_multi(
		...
		let cfg =
			Arc::new(Config { local_node: Some("local-node".to_string()), ..crate::config::parse_config().unwrap() });
		...
		let outbound = OutboundConnection {
			...
			id: TraceParent::new(),
			pool: pool::WorkloadHBONEPool::new(cfg.clone(), sock_fact, local_workload_information.clone()),
	...
	async fn build_request_unknown_dest() {
		run_build_request(
			...
			Some(ExpectedRequest { protocol: Protocol::TCP, hbone_destination: "", destination: "1.2.3.4:80" }),
		)
		.await;
	}
	...
	async fn build_request_known_dest_remote_node_tcp() {
		run_build_request(
			...
			Some(ExpectedRequest { protocol: Protocol::TCP, hbone_destination: "", destination: "127.0.0.2:80" }),
		)
		.await;
	}
		...
		run_build_request(
			...
			XdsAddressType::Workload(XdsWorkload {
				...
				waypoint: Some(xds::istio::workload::GatewayAddress {
					destination: Some(xds::istio::workload::gateway_address::Destination::Address(XdsNetworkAddress {
						network: "".to_string(),
			...
			Some(ExpectedRequest { protocol: Protocol::TCP, hbone_destination: "", destination: "127.0.0.1:80" }),
		)
		.await;
	}
	...
	async fn build_request_destination_svc_waypoint() {
		run_build_request(
			...
			XdsAddressType::Service(XdsService {
				addresses: vec![XdsNetworkAddress { network: "".to_string(), address: vec![127, 0, 0, 3] }],
			...
			vec![
				...
				XdsAddressType::Workload(XdsWorkload {
					...
					services: std::collections::HashMap::from([(
						...
						PortList { ports: vec![Port { service_port: 80, target_port: 1234 }] },
					)]),
					..Default::default()
				}),
				...
				XdsAddressType::Workload(XdsWorkload {
					...
					services: std::collections::HashMap::from([(
						"/example.com".to_string(),
						PortList { ports: vec![Port { service_port: 8080, target_port: 9999 }] },
					)]),
					..Default::default()
				}),
			],
			Some(ExpectedRequest { protocol: Protocol::TCP, hbone_destination: "", destination: "127.0.0.2:1234" }),
		)
		.await;
	}
	...
	async fn build_request_host_network() {
		let xds = vec![
			...
			XdsAddressType::Service(XdsService {
				hostname: "example.com".to_string(),
				addresses: vec![XdsNetworkAddress { network: "".to_string(), address: vec![127, 0, 0, 3] }],
				ports: vec![Port { service_port: 80, target_port: 80 }],
				..Default::default()
			}),
			...
			XdsAddressType::Workload(XdsWorkload {
				...
				services: std::collections::HashMap::from([(
					"/example.com".to_string(),
					PortList { ports: vec![Port { service_port: 80, target_port: 80 }] },
				)]),
		...
		let res = run_build_request_multi(
			...
			Some(ExpectedRequest { destination: "127.0.0.2:80", protocol: Protocol::TCP, hbone_destination: "" }),
		)
		.await
		.expect("must resolve");
		// Ensure it actually went to pod1, not the other pod with the same IP
		assert_eq!(res.actual_destination_workload.expect("found a dest").name, "pod1");
	...
	async fn multiple_address_workload() {
		...
		run_build_request(
			...
			"[ff06::c3]:80",
			workload.clone(),
			Some(ExpectedRequest { protocol: Protocol::TCP, hbone_destination: "", destination: "[ff06::c3]:80" }),
		)
		.await;
	}
	...
	async fn service_ip_families() {
		...
		let workload = XdsAddressType::Workload(XdsWorkload {
			...
			tunnel_protocol: 1,
			services: std::collections::HashMap::from([("/example.com".to_string(), PortList { ports: vec![] })]),
			..Default::default()
		});
		let svc = |f: IpFamilies| {
			let mut s = XdsService {
				hostname: "example.com".to_string(),
				addresses: vec![
					XdsNetworkAddress { network: "".to_string(), address: vec![127, 0, 0, 3] },
					XdsNetworkAddress {
						network: "".to_string(),
