...
impl H2ConnectClient {
	...
	pub async fn send_request(&mut self, req: http::Request<()>) -> Result<crate::proxy::h2::H2Stream, Error> {
		...
		let read = crate::proxy::h2::H2StreamReadHalf { recv_stream: recv, _dropped: dropped1 };
		let write = crate::proxy::h2::H2StreamWriteHalf { send_stream: send, _dropped: dropped2 };
		let h2 = crate::proxy::h2::H2Stream { read, write };
		Ok(h2)
	}

	// helper to allow us to handle errors once
	async fn internal_send(&mut self, req: Request<()>) -> Result<(SendStream<Bytes>, h2::RecvStream), Error> {
...
pub async fn spawn_connection(
	...
	let c = H2ConnectClient { sender: send_req, stream_count: Arc::new(AtomicU16::new(0)), max_allowed_streams };
	Ok(c)
}
