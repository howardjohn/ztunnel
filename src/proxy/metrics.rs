...
impl Metrics {
	pub fn new(registry: &mut Registry) -> Self {
		...
		Self { connection_opens, connection_close, received_bytes, sent_bytes, on_demand_dns }
	}
}
...
// access logs/metrics
pub fn log_early_deny<E: std::error::Error>(src: SocketAddr, dst: SocketAddr, reporter: Reporter, err: E) {
	event!(
			target: "access",
			parent: None,
			tracing::Level::WARN,
...
impl ConnectionResult {
	pub fn new(
		...
		let mut dst = (dst, conn.destination.as_ref().map(|wl| wl.name.clone().into()));
		...
		Self { src, dst, hbone_target, start, tl, metrics, sent, sent_metric, recv, recv_metric, recorded: false }
	}
	...
	pub fn record_with_flag<E: std::error::Error>(mut self, res: Result<(), E>, flag: ResponseFlags) {
	...
	fn record_internal<E: std::error::Error>(&mut self, res: Result<(), E>) {
		...
		let mtls = tl.connection_security_policy == SecurityPolicy::mutual_tls;
		let bytes = (self.recv.load(Ordering::SeqCst), self.sent.load(Ordering::SeqCst));
