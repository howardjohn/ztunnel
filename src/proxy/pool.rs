...
impl ConnSpawner {
	async fn new_pool_conn(&self, key: WorkloadKey) -> Result<ConnClient, Error> {
		...
		let sender = h2::client::spawn_connection(self.cfg.clone(), tls_stream, self.timeout_rx.clone()).await?;
		let client = ConnClient { sender, wl_key: key };
		Ok(client)
	}
}
...
impl PoolState {
	...
	fn guarded_get(&self, hash_key: &u64, workload_key: &WorkloadKey) -> Result<Option<ConnClient>, Error> {
	...
	fn enforce_key_integrity(conn: ConnClient, expected_key: &WorkloadKey) -> Result<ConnClient, Error> {
...
impl WorkloadHBONEPool {
	...
	pub fn new(
		...
		let spawner = ConnSpawner { cfg, socket_factory, local_workload, timeout_rx: timeout_recv.clone() };
...
mod test {
	...
	macro_rules! assert_opens_drops {
		($srv:expr, $open:expr, $drops:expr) => {
			assert_eq!($srv.conn_counter.load(Ordering::Relaxed), $open, "total connections opened, wanted {}", $open);
			#[allow(clippy::reversed_empty_ranges)]
			for want in 0..$drops {
				tokio::time::timeout(Duration::from_secs(2), $srv.drop_rx.recv())
					.await
					.expect(&format!("wanted {} drops, but timed out after getting {}", $drops, want))
					.expect("wanted drop");
			}
			assert!(
	...
	async fn spawn_clients_concurrently(
		...
		for req_num in 0..req_count {
			...
			debug!("client spent {}ms waiting for conn", start.elapsed().as_millis());
	...
	async fn spawn_server(
		...
		async fn hello_world(req: Request<Incoming>) -> Result<Response<Empty<bytes::Bytes>>, Infallible> {
			...
			let accept = async move {
				loop {
					...
					let server = crate::hyper_util::http2_server()
						...
						.serve_connection(hyper_util::rt::TokioIo::new(stream), service_fn(hello_world));
	...
	async fn setup_test_with_idle(max_conns: u16, idle: Duration) -> (WorkloadHBONEPool, TestServer) {
		...
		let server = TestServer { conn_counter, drop_rx, goaway_tx, addr };
		(pool, server)
	}
