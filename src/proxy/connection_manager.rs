...
impl ConnectionManager {
	pub fn track_outbound(
		...
		let c = OutboundConnection { src, original_dst, actual_dst };
		...
		OutboundConnectionGuard { cm: self.clone(), conn: c }
	}
	...
	pub async fn assert_rbac(
		...
		// track()
		let conn = InboundConnection { ctx: ctx.clone(), dest_service };
		...
		Ok(ConnectionGuard { cm: self.clone(), conn, watch: Some(watch) })
	}
...
impl PolicyWatcher {
	pub fn new(state: DemandProxyState, stop: DrainWatcher, connection_manager: ConnectionManager) -> Self {
		PolicyWatcher { state, stop, connection_manager }
	}
...
mod tests {
	...
	async fn test_connection_manager_close() {
		...
		let register = |cm: &ConnectionManager, c: &InboundConnection| {
			...
			ConnectionGuard { cm, conn: c, watch: Some(watch) }
		};
	...
	async fn test_policy_watcher_lifecycle() {
		...
		// this block will scope our guard appropriately
		{
			...
			let res = state_mutator.insert_authorization(&mut s, auth_xds_name.clone().into(), auth);
