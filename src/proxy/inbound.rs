...
impl Inbound {
	pub(super) async fn new(pi: Arc<ProxyInputs>, drain: DrainWatcher) -> Result<Inbound, Error> {
		...
		Ok(Inbound { listener, drain, pi, enable_orig_src })
	}
	...
	pub(super) async fn run(self) {
		...
		let acceptor = InboundCertProvider { local_workload: self.pi.local_workload_information.clone() };
			...
			async move {
				while let Some(tls) = stream.next().await {
					...
					let serve_client = async move {
						...
						let request_handler =
							move |req| Self::serve_connect(pi.clone(), conn.clone(), self.enable_orig_src, req);
						let serve =
							Box::pin(h2::server::serve_connection(cfg, tls, drain, force_shutdown, request_handler));
						serve.await
					};
		...
		run_with_drain("inbound".to_string(), self.drain, pi.cfg.self_termination_deadline, accept).await
	}
	...
	async fn serve_connect(pi: Arc<ProxyInputs>, conn: Connection, enable_original_source: bool, req: H2Request) {
		let src = conn.src;
		let dst = conn.dst;
		...
		let rx = async {
			let conn_guard = pi
				...
				.await
				.map_err(InboundFlagError::build(StatusCode::UNAUTHORIZED, ResponseFlags::AuthorizationPolicyDenied))?;

			let orig_src = enable_original_source.then_some(ri.rbac_ctx.conn.src.ip());
			let stream = super::freebind_connect(orig_src, ri.upstream_addr, pi.socket_factory.as_ref())
				.await
				.map_err(Error::ConnectionFailed)
				.map_err(InboundFlagError::build(StatusCode::SERVICE_UNAVAILABLE, ResponseFlags::ConnectionFailure))?;
		...
		let send = req
			...
			.and_then(|h2_stream| async {
				if ri.inbound_protocol == AppProtocol::PROXY {
					let Connection { src, src_identity, .. } = ri.rbac_ctx.conn;
					super::write_proxy_protocol(&mut stream, (src, ri.hbone_addr), src_identity)
						.instrument(trace_span!("proxy protocol"))
						.await?;
				}
				copy::copy_bidirectional(h2_stream, copy::TcpStreamSplitter(stream), &ri.result_tracker)
	...
	async fn build_inbound_request(
		...
		let hbone_addr = req
			...
			.as_str()
			.parse::<SocketAddr>()
			.map_err(|_| InboundError(Error::ConnectAddress(req.uri().to_string()), StatusCode::BAD_REQUEST))?;
		...
		let conn = Connection { dst: upstream_addr, ..conn };

		let rbac_ctx = ProxyRbacContext { conn, dest_workload: destination_workload.clone() };
		...
		let from_gateway =
			proxy::check_from_network_gateway(&pi.state, &destination_workload, rbac_ctx.conn.src_identity.as_ref())
				.await;
		if from_gateway {
		...
		let ds = proxy::guess_inbound_service(&rbac_ctx.conn, &for_host, upstream_service, &destination_workload);
		...
		Ok(InboundRequest { for_host, rbac_ctx, result_tracker, upstream_addr, inbound_protocol, hbone_addr })
	}
	...
	async fn validate_destination(
		...
		let hbone_dst = &NetworkAddress { network: conn.dst_network.clone(), address: hbone_addr.ip() };
	...
	fn find_inbound_upstream(
		...
		let (upstream_addr, inbound_protocol) = match local_workload.application_tunnel.clone() {
			Some(workload::ApplicationTunnel { port, protocol }) => {
				...
				let new_target = SocketAddr::new(upstream_addr.ip(), port.unwrap_or(upstream_addr.port()));
...
mod tests {
	...
	use crate::{
		...
		state::{
			...
			workload::{
				application_tunnel::Protocol as AppProtocol, gatewayaddress::Destination, ApplicationTunnel,
				GatewayAddress, NetworkAddress, Protocol, Workload,
			},
			DemandProxyState,
		},
		test_helpers,
	...
	const APP_TUNNEL_PROXY: Option<ApplicationTunnel> =
		Some(ApplicationTunnel { port: Some(PROXY_PORT), protocol: AppProtocol::PROXY });
	...
	async fn test_find_inbound_upstream<'a>(
		...
		let local_wl = state
			.fetch_workload_by_address(&NetworkAddress { network: "".into(), address: conn.dst.ip() })
			.await
			.unwrap();
	...
	fn test_state(server_waypoint: Waypoint) -> anyhow::Result<state::DemandProxyState> {
		...
		let services =
			vec![("waypoint", WAYPOINT_SVC_IP, Waypoint::None), ("server", SERVER_SVC_IP, server_waypoint.clone())]
				.into_iter()
				.map(|(name, vip, waypoint)| Service {
					name: name.into(),
					namespace: "default".into(),
					hostname: strng::format!("{name}.default.svc.cluster.local"),
					vips: vec![NetworkAddress { address: vip.parse().unwrap(), network: "".into() }],
