...
impl InboundPassthrough {
	pub(super) async fn new(pi: Arc<ProxyInputs>, drain: DrainWatcher) -> Result<InboundPassthrough, Error> {
		let listener = pi
			.socket_factory
		...
		Ok(InboundPassthrough { listener, pi, drain, enable_orig_src })
	}
	...
	pub(super) async fn run(self) {
		...
		run_with_drain("inbound passthrough".to_string(), self.drain, pi.cfg.self_termination_deadline, accept).await
	}
	...
	async fn proxy_inbound_plaintext(
		...
		if illegal_call {
			metrics::log_early_deny(source_addr, dest_addr, Reporter::destination, Error::SelfCall);
			return;
		}
		...
		let derived_source =
			metrics::DerivedWorkload { identity: rbac_ctx.conn.src_identity.clone(), ..Default::default() };
		let ds = proxy::guess_inbound_service(&rbac_ctx.conn, &None, upstream_services, &upstream_workload);
			...
			.await
		{
			Ok(cg) => cg,
			Err(e) => {
				result_tracker.record_with_flag(Err(e), metrics::ResponseFlags::AuthorizationPolicyDenied);
				return;
			}
		};

		let orig_src = if enable_orig_src { Some(source_addr.ip()) } else { None };
