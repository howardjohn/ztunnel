...
pub fn tls_server<T: ServerCertProvider + Clone + 'static>(
	...
	tls_listener::builder(crate::tls::InboundAcceptor::new(cert_provider))
		.listen(listener)
		.take_while(
			|item| !matches!(item, Err(tls_listener::Error::ListenerError(e)) if proxy::util::is_runtime_shutdown(e)),
		)
		.filter_map(|conn| {
...
impl hyper::rt::Timer for TokioTimer {
	...
	fn sleep_until(&self, deadline: Instant) -> Pin<Box<dyn Sleep>> {
		Box::pin(TokioSleep { inner: Box::pin(tokio::time::sleep_until(deadline.into())) })
	}
}
...
impl<S> Server<S> {
	pub async fn bind(name: &str, addrs: config::Address, drain_rx: DrainWatcher, s: S) -> anyhow::Result<Self> {
		let mut binds = vec![];
		for addr in addrs.into_iter() {
			binds.push(TcpListener::bind(&addr).await?)
		}
		Ok(Server { name: name.to_string(), binds, drain_rx, state: s })
	}
