...
pub fn construct_config(pc: ProxyConfig) -> Result<Config, Error> {
	...
	let bind_wildcard =
		if ipv6_enabled { IpAddr::V6(Ipv6Addr::UNSPECIFIED) } else { IpAddr::V4(Ipv4Addr::UNSPECIFIED) };
	...
	let xds_root_cert_provider = parse_default(XDS_ROOT_CA_ENV, DEFAULT_ROOT_CERT_PROVIDER.to_string())?;
	...
	let ca_root_cert_provider = parse_default(CA_ROOT_CA_ENV, DEFAULT_ROOT_CERT_PROVIDER.to_string())?;
	...
	let auth = match std::fs::read(DEFAULT_TOKEN_PROVIDER) {
		Ok(_) => identity::AuthSource::Token(PathBuf::from(DEFAULT_TOKEN_PROVIDER), cluster_id.clone()),
	...
	let proxy_workload_information = if proxy_mode == ProxyMode::Dedicated {
		...
		Some(state::WorkloadInfo { name: name.to_string(), namespace: ns.to_string(), service_account: sa.to_string() })
	} else {
		None
	};
	...
	validate_config(Config {
		...
		pool_unused_release_timeout: match parse::<String>(POOL_UNUSED_RELEASE_TIMEOUT)? {
			Some(ttl) => {
				duration_str::parse(&ttl).map_err(|_| Error::EnvVar(POOL_UNUSED_RELEASE_TIMEOUT.to_string(), ttl))?
			}
		...
		admin_addr: Address::Localhost(ipv6_localhost_enabled, pc.proxy_admin_port.unwrap_or(DEFAULT_ADMIN_PORT)),
		stats_addr: Address::SocketAddr(SocketAddr::new(bind_wildcard, pc.stats_port.unwrap_or(DEFAULT_STATS_PORT))),
...
fn validate_config(cfg: Config) -> Result<Config, Error> {
	if cfg.dns_proxy && cfg.xds_on_demand {
		return Err(Error::ProxyConfig(anyhow!("DNS proxy does not currently support on-demand mode")));
	}

	if !cfg.proxy && !cfg.dns_proxy {
		return Err(Error::ProxyConfig(anyhow!("ztunnel run without any servers enabled")));
	}
...
fn construct_proxy_config(mc_path: &str, pc_env: Option<&str>) -> anyhow::Result<ProxyConfig> {
	...
	let proxy_config_env = pc_env
		.map(|pc_env| if pc_env.is_empty() { Ok(None) } else { serde_yaml::from_str(pc_env) })
		.unwrap_or(Ok(None))
...
impl Display for Address {
	fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
		match self {
			Address::Localhost(_, port) => {
				write!(f, "localhost:{port}")
			}
...
pub mod tests {
	...
	fn config_from_proxyconfig() {
		...
		let cfg = construct_config(pc).unwrap();
		assert_eq!(cfg.readiness_addr.port(), default_config.readiness_addr.port());
		assert_eq!(cfg.xds_address.unwrap(), "https://istiod-rev0.istio-system-2:15012");
