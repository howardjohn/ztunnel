...
use crate::state::workload::{gatewayaddress, GatewayAddress, NamespacedHostname, NetworkAddress, Workload};
...
pub fn test_config_with_waypoint(addr: IpAddr) -> config::Config {
	config::Config {
		local_xds_config: Some(ConfigSource::Static(local_xds_config(80, Some(addr), vec![]).unwrap())),
		..test_config()
	}
}
...
pub fn test_config_with_port_xds_addr_and_root_cert(
	...
	let mut cfg = config::Config {
		...
		local_xds_config: match xds_config {
			Some(c) => Some(c),
			None => Some(ConfigSource::Static(local_xds_config(port, None, vec![]).unwrap())),
		},
...
pub fn localhost_error_message() -> String {
	let addrs = &[TEST_WORKLOAD_SOURCE, TEST_WORKLOAD_HBONE, TEST_WORKLOAD_TCP, TEST_VIP];
	format!(
...
pub fn mock_default_service() -> Service {
	let vip1 = NetworkAddress { address: IpAddr::V4(Ipv4Addr::new(127, 0, 10, 1)), network: strng::EMPTY };
	let vips = vec![vip1];
...
fn test_custom_svc(
	...
	Ok(Service {
		...
		vips: vec![NetworkAddress { network: strng::EMPTY, address: vip.parse()? }],
...
pub fn local_xds_config(
	...
	let lc = LocalConfig { workloads: res, policies, services: svcs };
...
pub fn new_proxy_state(
	...
	for w in xds_workloads {
		let res = XdsResource {
			...
			resource: XdsAddress { r#type: Some(address::Type::Workload(w.clone())) },
		};
	...
	for s in xds_services {
		let res = XdsResource {
			...
			resource: XdsAddress { r#type: Some(address::Type::Service(s.clone())) },
		};
		let handler = &updater as &dyn Handler<XdsAddress>;
		handler
			.handle(Box::new(&mut vec![XdsUpdate::Update(res)].into_iter()))
	...
	for a in xds_authorizations {
		let res = XdsResource { name: a.name.as_str().into(), resource: a.clone() };
	...
	DemandProxyState::new(state, None, ResolverConfig::default(), ResolverOpts::default(), metrics)
}
