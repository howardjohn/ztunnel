...
impl ProxyStateUpdateMutator {
	...
	pub fn new_no_fetch() -> Self {
		ProxyStateUpdateMutator { cert_fetcher: Arc::new(NoCertFetcher()) }
	}
	...
	for (namespaced_host, ports) in services {
		...
		let namespaced_host = match namespaced_host.split_once('/') {
			Some((namespace, hostname)) => {
				NamespacedHostname { namespace: namespace.into(), hostname: hostname.into() }
			}
			None => {
				return Err(anyhow::anyhow!("failed parsing service name: {namespaced_host}"));
			}
		};

		services_state.insert_endpoint(
			namespaced_host,
			Endpoint { workload_uid: workload.uid.clone(), port: ports.into(), status: workload.status },
		)
	}
	Ok(())
}
