...
use crate::state::service::{Endpoint, IpFamily, LoadBalancerMode, LoadBalancerScopes, ServiceStore};
use crate::state::service::{Service, ServiceDescription};
use crate::state::workload::{
	address::Address, gatewayaddress::Destination, network_addr, GatewayAddress, NamespacedHostname, NetworkAddress,
...
#[derive(Debug, Clone, Eq, Hash, Ord, PartialEq, PartialOrd, serde::Serialize, serde::Deserialize)]
...
impl fmt::Display for WorkloadInfo {
	fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
		write!(f, "{}.{} ({})", self.service_account, self.namespace, self.name)
	}
}

impl WorkloadInfo {
	pub fn new(name: String, namespace: String, service_account: String) -> Self {
		Self { name, namespace, service_account }
	}

	pub fn matches(&self, w: &Workload) -> bool {
		self.name == w.name && self.namespace == w.namespace && self.service_account == w.service_account
	}
}
...
impl serde::Serialize for ProxyState {
		...
		S: Serializer,
	{
		...
		let policies: Vec<_> = self
			...
			.collect();
		let serializable =
			ProxyStateSerialization { workloads, services, policies, staged_services: &self.services.staged_services };
...
impl ProxyState {
	...
	fn find_upstream(
		...
		{
			return self.find_upstream_from_service(source_workload, addr.port(), resolution_mode, svc);
		}
		if let Some(wl) = self
			.workloads
	...
	fn find_upstream_from_service(
		...
		let Some((ep, wl)) = self.load_balance(source_workload, &svc, svc_port, resolution_mode) else {
		...
		let target_port = if let Some(&ep_target_port) = ep.port.get(&svc_port) {
			...
			debug!("found service {}, but port {} was unknown", svc.hostname, svc_port);
			return None;
		};
			...
			match resolution_mode {
				...
				ServiceResolutionMode::Waypoint => {
					if target_port.is_none() && wl.application_tunnel.is_none() {
						...
						trace!("filter waypoint endpoint {}, target port is not defined", ep.workload_uid);
						return None;
					}
				}
			}
...
impl DemandProxyState {
	pub fn new(
		...
		Self { state, demand, dns_resolver, metrics }
	}
	...
	pub async fn assert_rbac(&self, ctx: &ProxyRbacContext) -> Result<(), proxy::AuthorizationRejectionError> {
		...
		trace!(allow = allow.len(), deny = deny.len(), "checking connection");
		...
		for pol in allow.iter() {
			if pol.matches(conn) {
				...
				trace!(policy = pol.to_key().as_str(), "allow policy does not match");
			}
		}
		// "Deny the request."
	...
	async fn pick_workload_destination_or_resolve(
		...
		if dst_workload.hostname.is_empty() {
			debug!("workload {} has no suitable workload IPs for routing", dst_workload.name);
			return Err(Error::NoValidDestination(Box::new(dst_workload.clone())));
		}
		let ip = Box::pin(self.resolve_workload_address(dst_workload, src_workload, original_target_address)).await?;
		Ok(ip)
	}
	...
	pub async fn wait_for_workload(&self, wl: &WorkloadInfo, deadline: Duration) -> Option<Arc<Workload>> {
	...
	async fn finalize_upstream(
		...
		let selected_workload_ip = self
			.pick_workload_destination_or_resolve(&wl, source_workload, original_target_address, ip_family_restriction)
		...
		let (res, target_address) = match &gw_address.destination {
			...
			Destination::Hostname(host) => {
				...
				match state.find_hostname(host) {
					...
					Some(_) => {
						return Err(Error::UnsupportedFeature("waypoint must be a service, not a workload".to_string()))
					}
					None => return Err(Error::UnknownWaypoint(format!("waypoint {} not found", host.hostname))),
				}
			}
		};
...
impl ProxyStateManager {
	pub async fn new(
		...
		let state: Arc<RwLock<ProxyState>> =
			Arc::new(RwLock::new(ProxyState::new(config.local_node.as_ref().map(strng::new))));
		let xds_client = if config.xds_address.is_some() {
			let updater = ProxyStateUpdater::new(state.clone(), cert_fetcher.clone());
			let tls_client_fetcher = Box::new(tls::ControlPlaneAuthentication::RootCert(config.xds_root_cert.clone()));
			Some(
...
mod tests {
	...
	async fn lookup_address() {
		...
		// Some from Address
		let dst =
			Destination::Address(NetworkAddress { network: strng::EMPTY, address: IpAddr::V4(Ipv4Addr::LOCALHOST) });
		test_helpers::assert_eventually(
			Duration::from_secs(5),
			|| mock_proxy_state.fetch_destination(&dst),
			Some(Address::Workload(Arc::new(test_helpers::test_default_workload()))),
		)
		.await;

		// Some from Hostname
		let dst =
			Destination::Hostname(NamespacedHostname { namespace: "default".into(), hostname: "defaulthost".into() });
		test_helpers::assert_eventually(
			Duration::from_secs(5),
			|| mock_proxy_state.fetch_destination(&dst),
			Some(Address::Service(Arc::new(test_helpers::mock_default_service()))),
		)
		.await;
		...
		test_helpers::assert_eventually(Duration::from_secs(5), || mock_proxy_state.fetch_destination(&dst), None)
			.await;

		// None from Hostname
		let dst = Destination::Hostname(NamespacedHostname { namespace: "default".into(), hostname: "nothost".into() });
		test_helpers::assert_eventually(Duration::from_secs(5), || mock_proxy_state.fetch_destination(&dst), None)
			.await;
	}
	...
	impl PortMappingTestCase {
		...
		fn app_tunnel(&self) -> Option<ApplicationTunnel> {
			if let PortMappingTestCase::AppTunnel = self {
				return Some(ApplicationTunnel { protocol: AppProtocol::PROXY, port: Some(15088) });
			}
			None
		}
	...
	async fn find_upstream_port_mappings(tc: PortMappingTestCase) {
		...
		let svc = Service {
			...
			namespace: "default".into(),
			vips: vec![NetworkAddress { address: "10.0.0.1".parse().unwrap(), network: "".into() }],
	...
	fn get_rbac_context(state: &DemandProxyState, dest_uid: u8, src_svc_acct: &str) -> crate::state::ProxyRbacContext {
		let key: Strng = format!("{}", dest_uid).into();
		...
		crate::state::ProxyRbacContext {
			conn: rbac::Connection {
				...
				src: std::net::SocketAddr::V4(SocketAddrV4::new(Ipv4Addr::new(192, 168, 1, 1), 1234)),
				dst_network: "".into(),
			...
			rbac::Authorization {
				...
				rules: vec![
					...
					vec![
						// from:
						vec![rbac::RbacMatch {
							principals: vec![StringMatch::Exact("cluster.local/ns/default/sa/defaultacct".into())],
							..Default::default()
						}],
					],
				],
			...
			rbac::Authorization {
				...
				rules: vec![
					...
					vec![
						// from:
						vec![rbac::RbacMatch {
							principals: vec![StringMatch::Exact("cluster.local/ns/default/sa/denyacct".into())],
							..Default::default()
						}],
					],
				],
	...
	async fn test_load_balance() {
		...
		let wl_match = Workload {
			...
			locality: Locality { region: "reg".into(), zone: "zone".into(), subzone: "".into() },
			..test_helpers::test_default_workload()
		};
		let wl_almost = Workload {
			...
			network: "network".into(),
			locality: Locality { region: "reg".into(), zone: "not-zone".into(), subzone: "".into() },
			..test_helpers::test_default_workload()
		};
		let _ep_almost = Workload {
			...
			network: "network".into(),
			locality: Locality { region: "reg".into(), zone: "other-not-zone".into(), subzone: "".into() },
			..test_helpers::test_default_workload()
		};
		let _ep_no_match = Workload {
			...
			locality: Locality { region: "not-reg".into(), zone: "unmatched-zone".into(), subzone: "".into() },
			..test_helpers::test_default_workload()
		};
		...
		assert_endpoint(&wl_no_locality, &strict_svc, vec![], "strict no match should not select");
		assert_endpoint(&wl_almost, &strict_svc, vec![], "strict no match should not select");
		assert_endpoint(&wl_match, &strict_svc, vec!["cluster1//v1/Pod/default/wl_match"], "strict match");
		...
		assert_endpoint(
			&wl_almost,
			&failover_svc,
			vec!["cluster1//v1/Pod/default/ep_almost", "cluster1//v1/Pod/default/wl_match"],
