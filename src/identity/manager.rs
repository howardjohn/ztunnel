...
impl SecretManager {
	pub async fn new(cfg: Arc<crate::config::Config>) -> Result<Self, Error> {
		let caclient = CaClient::new(
			...
			Box::new(tls::ControlPlaneAuthentication::RootCert(cfg.ca_root_cert.clone())),
			cfg.auth.clone(),
	...
	pub fn new_with_client<C: 'static + CaClientTrait>(client: C) -> Self {
		Self::new_internal(
			Box::new(client),
			SecretManagerConfig { time_conv: crate::time::Converter::new(), concurrency: 8 },
		)
		.0
	}

	fn new_internal(client: Box<dyn CaClientTrait>, cfg: SecretManagerConfig) -> (Self, tokio::task::JoinHandle<()>) {
		let (tx, rx) = mpsc::channel(10);
		let (worker, handle) = Worker::new(client, rx, cfg);
		(Self { worker, requests: tx }, handle)
	}
	...
	async fn start_fetch(&self, id: &Identity, pri: Priority) -> Result<watch::Receiver<CertState>, Error> {
	...
	async fn wait(&self, mut rx: watch::Receiver<CertState>) -> Result<Arc<tls::WorkloadCertificate>, Error> {
		loop {
			tokio::select! {
	...
	pub async fn fetch_certificate(&self, id: &Identity) -> Result<Arc<tls::WorkloadCertificate>, Error> {
...
pub mod mock {
	...
	pub fn new_secret_manager(cert_lifetime: Duration) -> Arc<SecretManager> {
		new_secret_manager_cfg(Config { cert_lifetime, fetch_latency: Duration::ZERO, epoch: None })
	}
	...
	pub fn new_secret_manager_cfg(cfg: Config) -> Arc<SecretManager> {
		...
		Arc::new(
			SecretManager::new_internal(Box::new(client), super::SecretManagerConfig { time_conv, concurrency: 2 }).0,
		)
	}
...
mod tests {
	...
	async fn test_cache_refresh() {
		...
		for _n in 0..3 {
			tokio::spawn(stress_single_id(secret_manager.clone(), id.clone(), test_dur));
		}

		tokio::time::timeout(Duration::from_secs(2), verify_cert_updates(secret_manager.clone(), id.clone()))
			.await
			.unwrap();
	}
	...
	fn setup(concurrency: u16) -> Test {
		...
		let (secret_manager, worker) =
			SecretManager::new_internal(Box::new(caclient.clone()), SecretManagerConfig { time_conv, concurrency });
		Test { worker, caclient, secret_manager: Arc::new(secret_manager) }
	}

	fn identity(name: &str) -> Identity {
		Identity::Spiffe { trust_domain: "test".into(), namespace: "test".into(), service_account: name.into() }
	}
	...
	async fn test_priority() {
		...
		let expected = vec![identity("id1"), identity("id4"), identity("id3"), identity("id2")];
		...
		assert_eq!(collect_strings(test.caclient.fetches().await), collect_strings(&expected),);
	...
	fn identity_from_string() {
		...
		assert_eq!(
			...
			Some(Identity::Spiffe { trust_domain: "td".into(), namespace: "ns".into(), service_account: "sa".into() })
		);
		assert_eq!(
		...
		assert_eq!(
			...
			Some(Identity::Spiffe { trust_domain: "td".into(), namespace: "".into(), service_account: "".into() })
		);
