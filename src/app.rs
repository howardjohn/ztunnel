...
pub async fn build_with_cert(config: Arc<config::Config>, cert_manager: Arc<SecretManager>) -> anyhow::Result<Bound> {
	...
	let proxy_task = if config.proxy { Some(ready.register_task("proxy")) } else { None };
	let dns_task = if config.dns_proxy { Some(ready.register_task("dns proxy")) } else { None };
	...
	let dns_metrics = if config.dns_proxy { Some(dns::Metrics::new(istio_registry)) } else { None };

	let (xds_tx, xds_rx) = tokio::sync::watch::channel(());
	// Create the manager that updates proxy state from XDS.
	let state_mgr =
		ProxyStateManager::new(config.clone(), xds_metrics, proxy_metrics.clone(), xds_tx, cert_manager.clone())
			.await?;
	...
	let mut admin_server =
		admin::Service::new(config.clone(), state.clone(), shutdown.trigger(), drain_rx.clone(), cert_manager.clone())
			.await
...
pub async fn build(config: Arc<config::Config>) -> anyhow::Result<Bound> {
	let cert_manager =
		if config.fake_ca { mock_secret_manager() } else { Arc::new(SecretManager::new(config.clone()).await?) };
...
fn init_inpod_proxy_mgr(
	...
	let metrics = Arc::new(crate::inpod::metrics::Metrics::new(registry.sub_registry_with_prefix("workload_manager")));
