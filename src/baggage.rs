...
pub fn parse_baggage_header(headers: GetAll<HeaderValue>) -> Result<Baggage, ToStrError> {
	let mut baggage = Baggage { ..Default::default() };
		...
		v.split(',').for_each(|s| {
			...
			if parts.len() > 1 {
				...
				match parts[0] {
					...
					"k8s.deployment.name" | "k8s.cronjob.name" | "k8s.pod.name" | "k8s.job.name" => {
						baggage.workload_name = val
					}
...
pub mod tests {
	...
	fn baggage_parser() -> anyhow::Result<()> {
		...
		let baggage_str =
			"k8s.cluster.name=K1,k8s.namespace.name=NS1,k8s.deployment.name=N1,service.name=N2,service.version=V1";
	...
	fn baggage_parser_multiline() -> anyhow::Result<()> {
		let mut hm = HeaderMap::new();
		hm.append(BAGGAGE_HEADER, HeaderValue::from_str("k8s.cluster.name=K1")?);
		hm.append(BAGGAGE_HEADER, HeaderValue::from_str("k8s.namespace.name=NS1")?);
		hm.append(BAGGAGE_HEADER, HeaderValue::from_str("k8s.deployment.name=N1")?);
