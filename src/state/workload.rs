...
impl From<xds::istio::workload::Locality> for Locality {
	fn from(value: xds::istio::workload::Locality) -> Self {
		Locality { region: value.region.into(), zone: value.zone.into(), subzone: value.subzone.into() }
	}
}
...
impl fmt::Display for Workload {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		write!(f, "Workload{{{} with uid {} ({:?})}}", self.name, self.uid, self.protocol)
	}
}
...
impl From<HashMap<u16, u16>> for PortList {
	fn from(value: HashMap<u16, u16>) -> Self {
		PortList {
			ports: value
				.iter()
				.map(|(k, v)| Port { service_port: *k as u32, target_port: *v as u32 })
				.collect(),
		}
	}
}
...
impl TryFrom<&XdsApplicationTunnel> for ApplicationTunnel {
	...
	fn try_from(value: &XdsApplicationTunnel) -> Result<Self, Self::Error> {
		Ok(ApplicationTunnel {
			protocol: application_tunnel::Protocol::from(xds::istio::workload::application_tunnel::Protocol::try_from(
				value.protocol,
			)?),
			port: if value.port == 0 { None } else { Some(value.port as u16) },
		})
	}
}
...
impl TryFrom<&XdsGatewayAddress> for GatewayAddress {
	...
	fn try_from(value: &xds::istio::workload::GatewayAddress) -> Result<Self, Self::Error> {
		let gw_addr: GatewayAddress = match &value.destination {
			Some(a) => match a {
				xds::istio::workload::gateway_address::Destination::Address(addr) => GatewayAddress {
				...
				xds::istio::workload::gateway_address::Destination::Hostname(hn) => GatewayAddress {
	...
	fn try_from(resource: XdsWorkload) -> Result<Self, Self::Error> {
		...
		let services: Vec<NamespacedHostname> = resource
			...
			.map(|namespaced_host| match namespaced_host.split_once('/') {
				...
				None => Err(WorkloadError::NamespacedHostnameParse(namespaced_host.clone())),
			})
			.collect::<Result<_, _>>()?;
		let wl = Workload {
			...
			protocol: Protocol::from(xds::istio::workload::TunnelProtocol::try_from(resource.tunnel_protocol)?),
			network_mode: NetworkMode::from(xds::istio::workload::NetworkMode::try_from(resource.network_mode)?),
			...
			status: HealthStatus::from(xds::istio::workload::WorkloadStatus::try_from(resource.status)?),
...
impl FromStr for NamespacedHostname {
	...
	fn from_str(value: &str) -> Result<Self, Self::Err> {
		...
		Ok(Self { namespace: namespace.into(), hostname: hostname.into() })
	}
}
		...
		D: Deserializer<'de>,
	{
		...
		impl<'de> Visitor<'de> for NetworkAddressVisitor {
				...
				E: serde::de::Error,
			{
				...
				Ok(NetworkAddress { network: network.into(), address: ip_addr })
			}
		}
...
pub fn network_addr(network: Strng, vip: IpAddr) -> NetworkAddress {
	NetworkAddress { network, address: vip }
}
...
impl From<&Identity> for WorkloadIdentity {
	fn from(value: &Identity) -> Self {
		let Identity::Spiffe { namespace, service_account, .. } = value;
		WorkloadIdentity { namespace: namespace.clone(), service_account: service_account.clone() }
	}
}
...
impl WorkloadStore {
	...
	pub fn remove(&mut self, uid: &Strng) -> Option<Workload> {
		match self.by_uid.remove(uid) {
			None => {
				trace!("tried to remove workload but it was not found; presumably it was a service");
				None
			}
			Some(prev) => {
...
mod tests {
	...
	fn byte_to_ipaddr_v6_happy() {
		let addr: Vec<u8> = Vec::from([32, 1, 13, 184, 133, 163, 0, 0, 0, 0, 138, 46, 3, 112, 115, 52]);
	...
	fn workload_information() {
		...
		let service1 = HashMap::from([(
			...
			XdsPortList { ports: vec![XdsPort { service_port: 80, target_port: 8080 }] },
		)]);
		...
		assert_eq!(state.read().unwrap().workloads.find_address(&nw_addr1), None);
		...
		updater
			.insert_service(
				...
				XdsService {
					...
					addresses: vec![XdsNetworkAddress { network: "".to_string(), address: vip1.octets().to_vec() }],
		...
		assert_eq!(
			...
			(state
				...
				.unwrap()
				.services
				.get_by_vip(&NetworkAddress { network: strng::EMPTY, address: IpAddr::V4(vip1) })
				.unwrap()),
		);
		...
		updater.remove(&mut state.write().unwrap(), &"ns/svc1.ns.svc.cluster.local".into());
		assert_eq!(state.read().unwrap().services.num_vips(), 0);
		assert_eq!((state.read().unwrap().services.num_services()), 0);
	}
	...
	fn unhealthy_workloads_staged() {
		...
		let services = HashMap::from([
			("ns/svc-normal".to_string(), XdsPortList { ports: vec![XdsPort { service_port: 80, target_port: 8080 }] }),
			(
		...
		updater
			.insert_service(
				...
				XdsService {
					...
					addresses: vec![XdsNetworkAddress { network: "".to_string(), address: vip2.octets().to_vec() }],
					ports: vec![XdsPort { service_port: 80, target_port: 80 }],
					subject_alt_names: vec![],
					waypoint: None,
		...
		let svc = state
			...
			.unwrap()
			.services
			.get_by_namespaced_host(&NamespacedHostname { namespace: "ns".into(), hostname: "svc-normal".into() });
	...
	fn unhealthy_workloads() {
		...
		let assert = |host: &str, want: usize| {
			let s = state
				...
				.unwrap()
				.services
				.get_by_namespaced_host(&NamespacedHostname { namespace: "ns".into(), hostname: host.into() });
			assert_eq!(s.unwrap().endpoints.inner.len(), want, "host {host} wanted {want}");
		};
		...
		let mut swapped = svc.clone();
		swapped.load_balancing =
			Some(LoadBalancing { routing_preference: vec![], mode: 0, health_policy: HealthPolicy::AllowAll as i32 });
		updater
			.insert_service(&mut state.write().unwrap(), swapped)
			.unwrap();
	...
	fn setup_test() -> (Arc<RwLock<ProxyState>>, DemandProxyState, ProxyStateUpdateMutator) {
		initialize_telemetry();
		let state = Arc::new(RwLock::new(ProxyState::new(None)));
		let mut registry = Registry::default();
		let metrics = Arc::new(crate::proxy::Metrics::new(&mut registry));
		let demand =
			DemandProxyState::new(state.clone(), None, ResolverConfig::default(), ResolverOpts::default(), metrics);
	...
	fn assert_vips(state: &DemandProxyState, want: Vec<&str>) {
		...
		let wl: Workload = (XdsWorkload { name: "some name".into(), ..Default::default() })
			.try_into()
			.unwrap();
		for _ in 0..1000 {
			if let Some((workload, _, _)) = state.state.read().unwrap().find_upstream(
				...
				wants.remove(&n.to_string());
				// remove using the borrow
			}
		}
		if !wants.is_empty() {
	...
	async fn local_client() {
		...
		assert_eq!(port, 8080);
		assert_eq!(svc.unwrap().subject_alt_names, vec!["spiffe://cluster.local/ns/default/sa/local".to_string()]);
