...
use crate::state::workload::{
	byte_to_ip, network_addr, GatewayAddress, NamespacedHostname, NetworkAddress, Workload, WorkloadError,
};
...
impl From<xds::istio::workload::load_balancing::Mode> for LoadBalancerMode {
	fn from(value: xds::istio::workload::load_balancing::Mode) -> Self {
		match value {
			...
			xds::istio::workload::load_balancing::Mode::UnspecifiedMode => LoadBalancerMode::Standard,
		}
	}
}
...
impl From<xds::istio::workload::load_balancing::HealthPolicy> for LoadBalancerHealthPolicy {
	fn from(value: xds::istio::workload::load_balancing::HealthPolicy) -> Self {
		match value {
			xds::istio::workload::load_balancing::HealthPolicy::OnlyHealthy => LoadBalancerHealthPolicy::OnlyHealthy,
			xds::istio::workload::load_balancing::HealthPolicy::AllowAll => LoadBalancerHealthPolicy::AllowAll,
		}
	}
}
...
impl TryFrom<&XdsService> for Service {
	...
	fn try_from(s: &XdsService) -> Result<Self, Self::Error> {
		...
		for addr in &s.addresses {
			let network_address =
				network_addr(strng::new(&addr.network), byte_to_ip(&Bytes::copy_from_slice(&addr.address))?);
		...
		let lb = if let Some(lb) = &s.load_balancing {
			Some(LoadBalancer {
				...
				health_policy: xds::istio::workload::load_balancing::HealthPolicy::try_from(lb.health_policy)?.into(),
			})
		} else {
			None
		};
		...
		let svc = Service {
			...
			vips: nw_addrs,
			ports: (&PortList { ports: s.ports.clone() }).into(),
