...
impl Service {
	pub async fn new(
		...
		Server::<State>::bind(
			...
			config.admin_addr,
			drain_rx,
			State { config, proxy_state, shutdown_trigger, cert_manager, handlers: vec![] },
		)
		.await
		.map(|s| Service { s })
	}
...
async fn handle_dashboard(_req: Request<Incoming>) -> Response<Full<Bytes>> {
	let apis = &[
		...
		("debug/pprof/heap", "collect heap profiling data (if supported, requires jmalloc)"),
...
async fn dump_certs(cert_manager: &SecretManager) -> Vec<CertsDump> {
	let mut dump = cert_manager
		.collect_certs(|id, certs| {
			let mut dump = CertsDump { identity: id.to_string(), ..Default::default() };
	...
	if let Some(cfg) = dump.config.local_xds_config.clone() {
		match cfg.read_to_string().await {
			Ok(data) => match serde_yaml::from_str(&data) {
				...
				Err(e) => {
					error!("Failed to load static workloads from local XDS {:?}:{:?}", dump.config.local_xds_config, e)
				}
			},
			Err(e) => {
				error!("Failed to read local XDS config {:?}:{:?}", dump.config.local_xds_config, e)
			}
		}
	}
...
async fn handle_jemalloc_pprof_heapgen(_req: Request<Incoming>) -> anyhow::Result<Response<Full<Bytes>>> {
...
mod tests {
	...
	async fn test_dump_certs() {
		...
		assert_eq!(got, want, "Certificate lists do not match (-want, +got):\n{}", diff_json(&want, &got));
	...
	async fn test_dump_config() {
		...
		let svc = XdsService {
			...
			addresses: vec![XdsNetworkAddress { network: "defaultnw".to_string(), address: [127, 0, 1, 1].to_vec() }],
			ports: vec![XdsPort { service_port: 80, target_port: 80 }],
			subject_alt_names: vec!["SAN1".to_string(), "SAN2".to_string()],
			waypoint: None,
			load_balancing: Some(XdsLoadBalancing { routing_preference: vec![1, 2], mode: 1, health_policy: 1 }), // ..Default::default() // intentionally don't default. we want all fields populated
			ip_families: 0,
		};
			...
			rules: vec![XdsRule {
				clauses: vec![XdsClause {
					matches: vec![XdsMatch {
						...
						source_ips: vec![XdsAddress { address: Bytes::copy_from_slice(&[127, 0, 0, 2]), length: 32 }],
						...
						namespaces: vec![XdsStringMatch { match_type: Some(XdsMatchType::Exact("ns".to_string())) }],
						not_namespaces: vec![XdsStringMatch {
							match_type: Some(XdsMatchType::Exact("not-ns".to_string())),
						}],
						principals: vec![XdsStringMatch {
							match_type: Some(XdsMatchType::Exact("spiffe://cluster.local/ns/ns/sa/sa".to_string())),
						}],
						not_principals: vec![XdsStringMatch {
							match_type: Some(XdsMatchType::Exact("spiffe://cluster.local/ns/ns/sa/not-sa".to_string())),
						}],
					}],
				}],
			}],
		...
		let default_config =
			construct_config(ProxyConfig::default()).expect("could not build Config without ProxyConfig");
	...
	async fn test_change_log_level() {
		...
		assert_eq!(resp_str, "current log level is hickory_server::server::server_future=off,info\n");
		...
		let resp_str = get_response_str(resp).await;
		assert!(resp_str.contains(HELP_STRING), "got {resp_str} want {HELP_STRING}");

		let resp = change_log_level(true, "debug");
		let resp_str = get_response_str(resp).await;
		assert_eq!(resp_str, "current log level is hickory_server::server::server_future=off,debug\n");

		let resp = change_log_level(true, "access=debug,info");
		let resp_str = get_response_str(resp).await;
		assert_eq!(resp_str, "current log level is hickory_server::server::server_future=off,access=debug,info\n");

		let resp = change_log_level(true, "warn");
		let resp_str = get_response_str(resp).await;
		assert_eq!(resp_str, "current log level is hickory_server::server::server_future=off,warn\n");

		let resp = change_log_level(true, "error");
		let resp_str = get_response_str(resp).await;
		assert_eq!(resp_str, "current log level is hickory_server::server::server_future=off,error\n");

		let resp = change_log_level(true, "trace");
		let resp_str = get_response_str(resp).await;
		assert!(resp_str.contains("current log level is hickory_server::server::server_future=off,trace\n"));

		let resp = change_log_level(true, "info");
		let resp_str = get_response_str(resp).await;
		assert!(resp_str.contains("current log level is hickory_server::server::server_future=off,info\n"));

		let resp = change_log_level(true, "off");
		let resp_str = get_response_str(resp).await;
		assert!(resp_str.contains("current log level is hickory_server::server::server_future=off,off\n"));
	}
}
