...
impl LocalWorkloadInformation {
	pub fn new(
		...
		LocalWorkloadInformation { wi, state, full_cert_manager: cert_manager }
	}
	...
	pub async fn fetch_certificate(&self) -> Result<Arc<tls::WorkloadCertificate>, identity::Error> {
...
async fn get_workload(state: &DemandProxyState, wi: Arc<WorkloadInfo>) -> Result<Arc<Workload>, Error> {
	state
...
impl ProxyInputs {
	pub fn new(
		...
		Arc::new(Self { cfg, state, metrics, connection_manager, socket_factory, local_workload_information, resolver })
	}
}

impl Proxy {
	#[allow(unused_mut)]
	pub(super) async fn from_inputs(mut pi: Arc<ProxyInputs>, drain: DrainWatcher) -> Result<Self, Error> {
		...
		let policy_watcher = PolicyWatcher::new(pi.state.clone(), drain, pi.connection_manager.clone());

		Ok(Proxy { inbound, inbound_passthrough, outbound, socks5, policy_watcher })
	}
...
pub async fn write_proxy_protocol<T>(stream: &mut TcpStream, addresses: T, src_id: Option<Identity>) -> io::Result<()>
where
	T: Into<ppp::v2::Addresses> + std::fmt::Debug,
{
	...
	let mut builder = Builder::with_addresses(Version::Two | Command::Proxy, Protocol::Stream, addresses);
...
impl TraceParent {
	fn new() -> Self {
		...
		Self { version: 0, trace_id: rng.gen(), parent_id: rng.gen(), flags: 0 }
	}
}

impl fmt::Debug for TraceParent {
	fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
		write!(f, "{:02x}-{:032x}-{:016x}-{:02x}", self.version, self.trace_id, self.parent_id, self.flags)
	}
}
...
pub(super) fn maybe_set_transparent(pi: &ProxyInputs, listener: &socket::Listener) -> Result<bool, Error> {
...
async fn check_from_waypoint(
	...
	let is_waypoint = |wl: &Workload| Some(wl.identity()).as_ref() == src_identity && wl.workload_ips.contains(src_ip);
...
mod tests {
	...
	async fn check_gateway() {
		...
		assert!(check_from_network_gateway(&state, &upstream_with_address, from_gw_conn.as_ref(),).await);
		assert!(!check_from_network_gateway(&state, &upstream_with_address, not_from_gw_conn.as_ref(),).await);

		// using hostname (will check the service variant of address::Address)
		let upstream_with_hostname = mock_wokload_with_gateway(Some(mock_default_gateway_hostname()));
		assert!(check_from_network_gateway(&state, &upstream_with_hostname, from_gw_conn.as_ref(),).await);
		assert!(!check_from_network_gateway(&state, &upstream_with_hostname, not_from_gw_conn.as_ref()).await);
	}
	...
	fn mock_default_gateway_service() -> Service {
		let vip1 = NetworkAddress { address: IpAddr::V4(Ipv4Addr::new(127, 0, 10, 1)), network: "".into() };
		let vips = vec![vip1];
