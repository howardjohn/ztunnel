...
impl Display for Connection {
	fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
		write!(f, "{}({})->{}", self.src, OptionDisplay(&self.src_identity), self.dst)
	}
}
		...
		for rule in self.rules.iter() {
			...
			for clause in rule.iter() {
				...
				for mg in clause.iter() {
					...
					let mut m = true;
					m &= Self::matches_internal("destination_ip", &mg.destination_ips, &mg.not_destination_ips, |i| {
						i.contains(&conn.dst.ip())
					});
					m &= Self::matches_internal("source_ips", &mg.source_ips, &mg.not_source_ips, |i| {
					...
					m &= Self::matches_internal("principals", &mg.principals, &mg.not_principals, |p| {
						p.matches_principal(&id)
					});
					m &= Self::matches_internal("namespaces", &mg.namespaces, &mg.not_namespaces, |p| p.matches(&ns));
...
impl TryFrom<&XdsAddress> for IpNet {
	...
	fn try_from(resource: &XdsAddress) -> Result<Self, Self::Error> {
		Ok(IpNet::new(byte_to_ip(&resource.address)?, resource.length as u8)?)
	}
}
...
mod tests {
	...
	fn rbac_empty_policy() {
		assert!(!allow_policy("empty", vec![vec![vec![RbacMatch { ..Default::default() }]]]).matches(&plaintext_conn()));
	...
	fn rbac_nesting() {
		let pol = allow_policy(
			...
			vec![vec![
				vec![
					RbacMatch { namespaces: vec![StringMatch::Exact("a".into())], ..Default::default() },
					RbacMatch { namespaces: vec![StringMatch::Exact("b".into())], ..Default::default() },
				],
				vec![RbacMatch { destination_ports: vec![80], ..Default::default() }],
			]],
		);
	...
	fn rbac_multi_rule() {
		let pol = allow_policy(
			"nested",
			vec![
				vec![vec![RbacMatch { namespaces: vec![StringMatch::Exact("a".into())], ..Default::default() }]],
				vec![vec![RbacMatch { namespaces: vec![StringMatch::Exact("b".into())], ..Default::default() }]],
			],
		);
		// Can match either namespace...
		assert!(pol.matches(&Connection {
