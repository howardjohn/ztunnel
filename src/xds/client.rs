...
impl AdsClient {
	...
	pub fn demander(&self) -> Option<Demander> {
		if self.config.on_demand {
			Some(Demander { demand: self.state.demand_tx.clone() })
		} else {
			None
		}
	}
	...
	async fn run_internal(&mut self) -> Result<(), Error> {
		...
		let tls_grpc_channel =
			tls::grpc_connector(self.config.address.clone(), self.config.tls_builder.fetch_cert().await?)?;

		let ads_connection =
			AggregatedDiscoveryServiceClient::with_interceptor(tls_grpc_channel, self.config.auth.clone())
...
mod tests {
	...
	async fn verify_address(ip: IpAddr, expected_address: Option<XdsAddress>, source: &DemandProxyState) {
		...
		let ip_network_addr = NetworkAddress { network: strng::EMPTY, address: ip };
	...
	fn get_auth(i: usize) -> ProtoResource {
		...
		ProtoResource {
			...
			resource: Some(Any { type_url: AUTHORIZATION_TYPE.to_string(), value: addr.encode_to_vec() }),
			ttl: None,
			cache_control: None,
		}
	}
	fn get_address(i: usize, addr: std::net::IpAddr) -> ProtoResource {
		...
		ProtoResource {
			...
			aliases: vec![],
			version: "0.0.1".to_string(),
			resource: Some(Any { type_url: ADDRESS_TYPE.to_string(), value: addr.encode_to_vec() }),
			ttl: None,
			cache_control: None,
		}
	}
	...
	fn test_json_to_value() {
		...
		assert_eq!(
			...
			StructValue(prost_types::Struct {
				fields: vec![
					("app".to_string(), prost_types::Value { kind: Some(Kind::StringValue("foo".to_string())) }),
					("version".to_string(), prost_types::Value { kind: Some(Kind::StringValue("v1".to_string())) }),
				]
				.into_iter()
				.collect()
			}),
		...
		assert_eq!(
			...
			ListValue(prost_types::ListValue {
				values: vec![
					prost_types::Value { kind: Some(Kind::StringValue("foo".to_string())) },
					prost_types::Value { kind: Some(Kind::StringValue("bar".to_string())) },
				]
			})
		);
