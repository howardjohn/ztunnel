...
use criterion::{criterion_group, criterion_main, BenchmarkGroup, Criterion, SamplingMode, Throughput};
...
fn initialize_environment(
	...
	echo_mode: Mode,
	clients: usize,
) -> anyhow::Result<(WorkloadManager, SyncSender<usize>, Receiver<Result<(), io::Error>>)> {
...
pub fn throughput(c: &mut Criterion) {
	...
	fn run_throughput<T: Measurement>(c: &mut BenchmarkGroup<T>, name: &str, mode: WorkloadMode, clients: usize) {
		let (_manager, tx, ack) = initialize_environment(mode, TestTrafficMode::Request, Mode::Read, clients).unwrap();
	...
	for clients in [1, 2, 8] {
		run_throughput(&mut c, &format!("direct{clients}"), WorkloadMode::Direct, clients);
		run_throughput(&mut c, &format!("tcp{clients}"), WorkloadMode::TcpClient, clients);
		run_throughput(&mut c, &format!("hbone{clients}"), WorkloadMode::HBONE, clients);
	}
}
...
pub fn latency(c: &mut Criterion) {
	...
	fn run_latency<T: Measurement>(c: &mut BenchmarkGroup<T>, name: &str, mode: WorkloadMode) {
		let (_manager, tx, ack) = initialize_environment(mode, TestTrafficMode::Request, Mode::Read, 1).unwrap();
		c.bench_function(name, |b| {
			b.iter(|| {
...
fn next_ip_pair(ip_pair: (u8, u8)) -> (u8, u8) {
	...
	match (Ord::cmp(&ip_pair.0, &(MAX_HBONE_WORKLOADS - 1)), Ord::cmp(&ip_pair.1, &MAX_HBONE_WORKLOADS)) {
...
fn hbone_connection_config() -> ztunnel::config::ConfigSource {
	...
	let lc = ztunnel::xds::LocalConfig { workloads, policies: vec![], services: vec![] };
...
fn hbone_connections(c: &mut Criterion) {
	...
	let (echo_addr, ta) = rt.block_on(async move {
		...
		let config = test_helpers::test_config_with_port_xds_addr_and_root_cert(port, None, None, config_source);
